...
public sealed record TextureSummary(
    ...
    IReadOnlyList<string>? MissingSubsurface = null);
...
public static class PresetCatalog
{
    private static readonly Dictionary<string, ConversionPreset> Presets = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["SAM Light Balanced"] = new("SAM Light Balanced","SAM Light", "balanced", "smp"),
        ...
        ["UBE Athletic"]      = new("UBE Athletic",       "UBE",   "athletic", "smp+cbpc"),
        ...
        ["Serpentine Humanoid Balanced"] = new("Serpentine Humanoid Balanced", "Serpentine Humanoid", "balanced", "none"),
        ["Serpentine Humanoid Athletic"] = new("Serpentine Humanoid Athletic", "Serpentine Humanoid", "athletic", "none"),
        ["Serpentine Humanoid Lean"] = new("Serpentine Humanoid Lean", "Serpentine Humanoid", "lean", "none"),
        ["Serpentine Humanoid Zeroed"] = new("Serpentine Humanoid Zeroed", "Serpentine Humanoid", "zeroed", "none"),
        ...
        ["Feline Humanoid Balanced"] = new("Feline Humanoid Balanced", "Feline Humanoid", "balanced", "none"),
        ["Feline Humanoid Athletic"] = new("Feline Humanoid Athletic", "Feline Humanoid", "athletic", "none"),
        ["Feline Humanoid Lean"] = new("Feline Humanoid Lean", "Feline Humanoid", "lean", "none"),
        ["Feline Humanoid Zeroed"] = new("Feline Humanoid Zeroed", "Feline Humanoid", "zeroed", "none"),
        ["Canine Humanoid Balanced"] = new("Canine Humanoid Balanced", "Canine Humanoid", "balanced", "none"),
        ["Canine Humanoid Athletic"] = new("Canine Humanoid Athletic", "Canine Humanoid", "athletic", "none"),
        ["Canine Humanoid Lean"] = new("Canine Humanoid Lean", "Canine Humanoid", "lean", "none"),
        ["Canine Humanoid Zeroed"] = new("Canine Humanoid Zeroed", "Canine Humanoid", "zeroed", "none"),
        ["Draconic Humanoid Balanced"] = new("Draconic Humanoid Balanced", "Draconic Humanoid", "balanced", "none"),
        ["Draconic Humanoid Athletic"] = new("Draconic Humanoid Athletic", "Draconic Humanoid", "athletic", "none"),
        ["Draconic Humanoid Lean"] = new("Draconic Humanoid Lean", "Draconic Humanoid", "lean", "none"),
        ["Draconic Humanoid Zeroed"] = new("Draconic Humanoid Zeroed", "Draconic Humanoid", "zeroed", "none"),
        ["Vanilla to Vanilla Beast"] = new("Vanilla to Vanilla Beast", "Vanilla Beast", "balanced", "none"),
        ["Vanilla to Serpentine Humanoid"] = new("Vanilla to Serpentine Humanoid", "Serpentine Humanoid", "balanced", "none"),
        ...
        ["Vanilla to Feline Humanoid"] = new("Vanilla to Feline Humanoid", "Feline Humanoid", "balanced", "none"),
        ["Vanilla to Canine Humanoid"] = new("Vanilla to Canine Humanoid", "Canine Humanoid", "balanced", "none"),
        ["Vanilla to Draconic Humanoid"] = new("Vanilla to Draconic Humanoid", "Draconic Humanoid", "balanced", "none"),
...
internal sealed class StrategyMeshConversionService : IMeshConversionService
{
    ...
    private static readonly IReadOnlyDictionary<string, IReadOnlyDictionary<string, double>> TargetBodyTunings =
        new Dictionary<string, IReadOnlyDictionary<string, double>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Serpentine Humanoid"] = CreateTuning(("waist", 0.93d), ("pelvis", 1.07d), ("butt", 1.05d), ("thighs", 0.96d), ("calves", 0.95d)),
...
internal sealed class BasicTextureAnalysisService : ITextureAnalysisService
{
    ...
    public async Task<TextureSummary> AnalyzeAsync(ImportedArmor armor, CancellationToken cancellationToken)
    {
        ...
        var missingSubsurface = new List<string>();
        ...
        foreach (var diffuse in diffuseFiles)
        {
            ...
            var expectedSubsurface = baseName + "_sk.dds";
            if (!subsurfaceFiles.Any(sk => sk.Equals(expectedSubsurface, StringComparison.OrdinalIgnoreCase)))
            {
                missingSubsurface.Add(diffuse);
            }
        }
        ...
        return new TextureSummary(
            ...
            MissingSubsurface: missingSubsurface);
    }
    ...
    IScratchPluginGeneratorService? scratchPluginGen = null) : IExportService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        outputFiles.AddRange(generatedSubsurface);
        ...
        if (generatedSpecular.Count > 0 || generatedParallax.Count > 0 || generatedGlow.Count > 0 || generatedRoughness.Count > 0 || generatedSubsurface.Count > 0)
        {
            await File.AppendAllTextAsync(logPath,
                $"aux-stubs:specular={generatedSpecular.Count},parallax={generatedParallax.Count},glow={generatedGlow.Count},roughness={generatedRoughness.Count},subsurface={generatedSubsurface.Count}{Environment.NewLine}",
                cancellationToken);
        }
        ...
        var expectedGeneratedRootSupportFiles = BuildExpectedGeneratedRootSupportFiles(
            hasPluginArtifacts: pluginAnalysis.ScannedPlugins.Count > 0 || pluginAnalysis.ArmorAddons.Count > 0);
    ...
    /// <summary>
    /// Neutral subsurface scattering map (_sk.dds): pure black means no SSS contribution.
    /// Armor and hard-surface meshes do not exhibit sub-surface scattering so an all-black
    /// stub is the correct neutral value and avoids unintended skin-tone bleed.
    /// </summary>
    internal static byte[] BuildSubsurfaceMapDds(int width = 4, int height = 4)
        => BuildSolidColorDds(b: 0x00, g: 0x00, r: 0x00, a: 0xFF, width, height);

    /// <summary>
    /// Generates stub DDS files for specular (_s), parallax (_p), glow (_g), roughness (_r), and
    /// subsurface (_sk) textures that are referenced by a diffuse map but do not yet exist in the
    /// output directory.
    /// Returns a tuple of (specularPaths, parallaxPaths, glowPaths, roughnessPaths, subsurfacePaths).
    /// </summary>
    private static async Task<(IReadOnlyList<string> Specular, IReadOnlyList<string> Parallax, IReadOnlyList<string> Glow, IReadOnlyList<string> Roughness, IReadOnlyList<string> Subsurface)>
            ...
            CancellationToken cancellationToken)
    {
        ...
        var subsurfaceGenerated  = new List<string>();
        ...
        var missingSubsurfaceSet  = new HashSet<string>(textureSummary.MissingSubsurface  ?? [], StringComparer.OrdinalIgnoreCase);

        if (missingSpecularSet.Count == 0 && missingParallaxSet.Count == 0 && missingGlowSet.Count == 0 && missingRoughnessSet.Count == 0 && missingSubsurfaceSet.Count == 0)
        ...
        foreach (var texturePath in armor.TextureFiles)
        {
            ...
            if (missingSubsurfaceSet.Contains(fileName))
            {
                var stubPath = Path.Combine(destDir, stemName + "_sk.dds");
                if (!File.Exists(stubPath))
                {
                    Directory.CreateDirectory(destDir);
